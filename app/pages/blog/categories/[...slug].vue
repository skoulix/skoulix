<template>
  <section class="p-section contain-xl space-y-12">
    <h1 class="h1">Articles under the "{{ slug }}" category.</h1>
    ...
    <EmptyState v-else title="No posts found.">
      <template #body>
        <p>We couldn't find any posts under the "{{ slug }}" category.</p>
      </template>
    </EmptyState>
  </section>
</template>
...
const { data: posts } = await useAsyncData(
  () => `category-posts-${slug.value}-${locale.value}-${currentPage.value}`,
  () =>
    queryCollection('blog')
      .where('locale', '=', locale.value)
      .where('category', '=', slug.value)
      ...
      .all(),
  {
    watch: [slug, locale, currentPage, perPage],
  },
);
